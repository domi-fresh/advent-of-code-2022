...
console.log("Challenge 1: Maximum releaseable pressure in 30min by yourself: " + idealWay[0])
...
function bruteForceBoth(mytime, eletime, mytar, eletar, mytimetotar, eletimetotar, valves, valvesAlreadyOpened, flowSoFar) {
    ...
    let msg = []
    let oldmsg = []
    let restFlow = [...valves].filter(e => valvesAlreadyOpened.indexOf(e) == -1).map(e => e.flowRate).sort((a, b) => b - a).reduce((acc, e, i) => { return acc + e * Math.max(0, (Math.max(mytime, eletime) - 2 * i)) }, 0) + flowSoFar

    if (restFlow <= bestFlow) {
        return [-1, []]
    }
    ...
    if (mytime > 1 && mytimetotar <= 0 && eletime > 1 && eletimetotar <= 0) {
        ...
        mydistances.forEach(valve => {
            if (valvesAlreadyOpened.indexOf(valve[0]) == -1) {
                // console.log(valve[0])
                ...
                eledistances.forEach(valve => {
                    if (valvesAlreadyOpened.indexOf(valve[0]) == -1) {
                        ...
                        // console.log({timeskip})
                        let possibleRelease = bruteForceBoth(mynewtime - timeskip, elenewtime - timeskip, mynewtar, elenewtar, mynewtimetotar - timeskip, elenewtimetotar - timeskip, [...valves], [...valvesAlreadyOpened], addedFlow + flowSoFar)
                        if (possibleRelease[0] > bestRelease) {
                            bestRelease = possibleRelease[0]
                            oldmsg = possibleRelease[1]
                        } else if (possibleRelease[0] == -1) {
            ...
            if (mytimetotar <= 0) {
                ...
                distances.forEach(valve => {
                    if (valvesAlreadyOpened.indexOf(valve[0]) == -1) {
                        ...
                        // console.log({timeskip})
                        let possibleRelease = bruteForceBoth(mynewtime - timeskip, eletime - timeskip, mynewtar, eletar, mynewtimetotar - timeskip, eletimetotar - timeskip, [...valves], [...valvesAlreadyOpened], addedFlow + flowSoFar)
            ...
            if (eletimetotar <= 0) {
                ...
                distances.forEach(valve => {
                    if (valvesAlreadyOpened.indexOf(valve[0]) == -1) {
                        ...
                        // console.log({timeskip})
                        let possibleRelease = bruteForceBoth(mytime - timeskip, elenewtime - timeskip, mytar, elenewtar, mytimetotar - timeskip, elenewtimetotar - timeskip, [...valves], [...valvesAlreadyOpened], addedFlow + flowSoFar)
                        if (possibleRelease[0] > bestRelease) {
                            bestRelease = possibleRelease[0]
                            oldmsg = possibleRelease[1]
                        } else if (possibleRelease[0] == -1) {
    ...
    bestFlow = Math.max(addedFlow + bestRelease, bestFlow)
    return [addedFlow + bestRelease, [...msg, ...oldmsg]]
}

let firsttarget = valveObjects[valveObjects.findIndex(e => e.name == "AA")]

console.time("find best pressure release")
let optimalPressureRelease = bruteForceBoth(26, 26, firsttarget, firsttarget, 0, 0, [...valveObjects], [...valvesWithoutFlow], 0)

console.log(optimalPressureRelease[1])
console.timeEnd("find best pressure release")
console.log("Challenge 2: with the elephant`s help you`re able to release " + optimalPressureRelease[0] + " pressure in 24min.")
